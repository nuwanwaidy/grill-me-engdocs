...
- A playbook file has been significantly changed
...
If fail: open the playbook's **Opening** section and **During [Anchor]**
instructions. The playbook must explicitly instruct Claude to name the most
...
If fail: open the playbook's anchor section for the question where the vague
...
If fail: open the playbook's **Before you begin** section. It must contain
...
- Manual spot-checks have been performed during playbook development
- Results are not tracked in `tests/results/` (the folder exists but is empty
  except for `.gitkeep`)
- Langfuse integration is not yet wired

**Practical implication:** universal grounding is enforced by the playbook
instructions (if written correctly) and verified manually during development.
It is not continuously verified. Any playbook change should trigger a manual
