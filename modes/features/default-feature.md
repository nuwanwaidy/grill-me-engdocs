...
- Rate each dimension relative to other features on the current roadmap:
  - Benefit (1–5): nice to have → unlocks blocked value stream
  - Penalty (1–5): no impact if missing → blocks TrustX contract
  - Cost (1–5): trivial → significant PRISM integration work
  - Risk (1–5): well understood → high dependency or uncertainty
...
> features on the current roadmap on Benefit · Penalty · Cost · Risk (each 1–5)."*
...
|---|---|---|---|---|---|---|
| Benefit | Value delivered if built | Nice to have | Useful | Meaningful | High value | Unlocks blocked value stream |
| Penalty | Cost of not building it | No impact if missing | Minor gap | Noticeable | Significant | Blocks TrustX data_trust contract |
| Cost | Engineering effort to build | Trivial — config change | Small | Moderate | Significant | Major PRISM integration work |
| Risk | Uncertainty and dependency | Well understood, no blockers | Low | Moderate | High | Blockers outside your control |
...
Every feature output must cite at least one applicable SIEM policy term.
...
| 2 | All sections present: Problem/Opportunity, Goal, User Impact, Scope, Success Criteria, Functional Specification | Yes |
| 3 | User Impact table has ≥1 row with before/after | Yes |
| 4 | REQ table has ≥1 requirement using "The system shall..." phrasing | Yes |
| 5 | REQ table has exactly two columns: ID and Requirement (no Status column) | Yes |
| 6 | Stimulus/Response sequences present with named actors | Yes |
| 7 | Sequence diagram section present | Yes |
| 8 | At least one `BusinessGoals.*` term named | Yes |
| 9 | At least one Workflow ID named | Yes |
| 10 | At least one SIEM policy term cited (SEC-POL-* · SEC-STD-* · SEC-RB-* · SEC-SOP-*) | Yes |
| 11 | RICE score computed (all four components present) | Yes |
| 12 | Wiegers value score computed (all four dimensions, 1–5 scale) | Yes |
| 13 | Handoff block present with all fields populated | Yes |
